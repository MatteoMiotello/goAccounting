...
func testUserToManyAssets(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadAssets(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpAssets(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `Username`: `character varying`, `Email`: `character varying`, `Password`: `character varying`, `DeletedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
